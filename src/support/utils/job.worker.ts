...
export class JobWorker {
  ...
  constructor(protected event: FlatfileEvent, protected job: Flatfile.Job) {
...
export class WorkbookJobWorker extends JobWorker {
  get workbookId(): string {
    if (!this.event.context.workbookId)
...
export class DocumentJobWorker extends JobWorker {
  get documentId(): string {
    console.log(this.event);
    if (!this.job.source) throw new Error("Document ID not found");
    return this.job.source;
  }

  async document(): Promise<Flatfile.Document> {
    const res = await api.documents.get(this.spaceId, this.documentId);
    return res.data;
  }
}
...
export function worker(Worker: typeof JobWorker) {
  ...
  if (!actions) {
    throw new Error(
      "You must add the @TriggeredBy(action) decorator to your worker definition."
    );
  }
