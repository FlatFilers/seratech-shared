...
export class MergeWorker extends SheetJobWorker {
  async execute(): Promise<void | JobOutcome> {
    ...
    const overrideKeys: string | string[] | undefined = this.job.input.on;

    const sheet = await this.sheet();

    const { duplicateRecords, mergedRecords } = MergeWorker.prepareMerge(
      sheet.config,
      records,
      overrideKeys
    );
    ...
    if (records.changes().count() > 0) {
      await safe.records.write(
        { sheetId: this.sheetId, snapshot: true },
        records
      );
    }

    return {
      message: `Successfully merged ${Math.max(
  ...
  public static prepareMerge(
    ...
    const mergeFieldKeys = sheet.fields
      .filter((f) => f.metadata?.is_dedupe_field)
      .map((f) => f.key);
...
export function mergeData(records: Collection<Item>, key?: RefType) {
  ...
  return {
    ...
    firstRecordOfEachHash: Array.from(uniques).map(([, ids]) =>
      result.get(ids[0])
    ),
  };
}
