...
export function instrumentRequests() {
  global.__instrumented =
    global.__instrumented === undefined ? false : global.__instrumented;
  ...
  if (!!process.env.AWS_LAMBDA_FUNCTION_NAME || process.env.CI === "true") {
    ...
    console.log = (farg, ...args) =>
      olog(typeof farg === "string" ? stripAnsi(farg) : farg, ...args);
  }
  ...
  function requestLogger(httpModule) {
    ...
    if (!httpModule.__instrumented) {
      ...
      httpModule.request = function (options, callback) {
        if (
          (options.href || options.host).includes("pndsn") ||
          (options.href || options.host).endsWith("/ack")
        ) {
        ...
        request.on("response", (response) => {
          response.on("end", () => {
            Debugger.logHttpRequest({
              ...
              url:
                options.href ||
                options.proto + "://" + options.host + options.path,
              startTime,
